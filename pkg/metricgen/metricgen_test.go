...
func TestMetricGenIterator(t *testing.T) {
	...
	tests := []struct {
		...
		{0, 253, base.Event{base.SEND, 0, "prefix.worker0.iter2 2 253\\n"}},
		{0, 254, base.Event{base.FLUSH, 1, "prefix.worker0.iter3 3 254\\n"}},
		{0, 123, base.Event{base.SEND, 0, "prefix.worker0.iter4 4 123\\n"}},
...
func BenchmarkMetricGenIteratorNext(b *testing.B) {
	workers := 16
	m := New("prefix", workers, 300000000, 1, 1, 1)
	t := time.Now().Unix()
	for n := 0; n < b.N; n++ {
		_ = m.Next(1, t)
	}
...
func BenchmarkMetricGenIteratorNextBatch(b *testing.B) {
	workers := 16
	m := New("prefix", workers, 300000000, 10, 10, 1)
	t := time.Now().Unix()
	for n := 0; n < b.N; n++ {
		_ = m.Next(1, t)
	}
